...
void avaliaCombinacao(ListaRochas* lista, int combinacao, float* valorTotal, float* pesoTotal) { // aqui na forca bruta ele combina tudo com tudo olhando peso e valor
    *valorTotal = 0;
    *pesoTotal = 0;
    for (int i = 0; i < lista->tamanho; i++) {
...
void melhor_rocha(ListaRochas* lista) {
    ...
    printf("Valor: %.2f, Peso: %2.f", melhorValor, melhorPeso);
    printf(" Solução:");
    printf("[");
    for (int i = 0; i < lista->tamanho; i++) {
